...
class Pce:
    ...
    def _getDeltaDailyCons(self,db,startStr,endStr):
        ...
        query = f"SELECT CASE COUNT(ALL) > 1 THEN max(value) - min(value) ELSE NULL END FROM consumption_daily WHERE pce = '{self.pceId}' AND date BETWEEN date({startStr}) AND date({endStr}) GROUP BY pce"
...
class DailyMeasure:
    ...
    def store(self,db):
        ...
        if self.isOk():
            logging.debug("Store measure %s, %s, %s m3, %s kWh",self.gasDate,self.endIndex, self.volume, self.energy)
