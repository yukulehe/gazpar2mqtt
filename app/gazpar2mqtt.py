    ...
    if myMqtt.isConnected \
        ...
        for myPce in myGrdf.pceList:
            ...
            if not myPce.isOk(): # Values when PCE is not correct
                ...
                # Create entities and set values
                myEntity = hass.Entity(myDevice,hass.SENSOR,'last_index','index',hass.ENERGY_TYPE,hass.ST_TTI,'m3').setValue(myDailyMeasure.endIndex)
                myEntity = hass.Entity(myDevice,hass.SENSOR,'daily_gas','daily gas',hass.ENERGY_TYPE,hass.ST_MEAS,'m3').setValue(myDailyMeasure.volume)
